...
TransmitChannel *wireless_channel = &loraChannel;
...
float wind_gust;
float precipitation;
String transmit_protocol = "lora";

void setup_routing() {     
  server.on("/data", HTTP_GET, getData);     
  server.on("/protocol", HTTP_POST, handleProtocolChange);
  server.on("/encryption", HTTP_POST, handleEncryptionChange);    
...
void handleEncryptionChange() {
  String body = server.arg("plain");
  Serial.print("Encryption ");Serial.println(body);
  wireless_channel->setEncryption(body=="on"?true:false);
